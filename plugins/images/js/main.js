...
$(document).ready(function(){
	//alert(window.opener.window.last);
...
function insertimage(c) 
{
// 	alert(opener.window.last);
	var textarea=$('#'+opener.window.last, opener.document);
	var value = textarea.val();
	var start = textarea[0].selectionStart;
	var end = textarea[0].selectionEnd;
	textarea.val(value.substr(0, start)+c+value.substring(start, end)+value.substring(end, value.length));
	textarea.focus();
}


function insertposter (c, field) 
{
	var textarea=$(field, opener.document);
	textarea.val(c);
	textarea.focus();
}
