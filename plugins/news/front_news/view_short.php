...
if (!defined('a1cms'))
	...
	$global_shortstory_tpl=get_template('shortstory_row');
	if($global_shortstory_tpl)
	{
	...
	// 				$data['parse']['{author_group_id}'] = $data['news_row']['user_group'];
				...
				$data['big_parse']=array('{cat-name}'=>$parse_main['{meta-title}'],'{rows}'=>$tmp_content);
				$short_text_content = parse_template(get_template('shortstory'),$data['big_parse']);
